...
public class PublicTimelineFragment extends StatusListFragment{
	...
	protected void doLoadData(int offset, int count){
		AccountSessionManager.getInstance()
				...
				.getPublicTimeline(offset>0 ? maxID : null, count, refreshing, new SimpleCallback<>(this){
					@Override
	...
	private void loadNewPosts(){
		...
		currentRequest=new GetPublicTimeline(false, false, null, sinceID, 20)
				.setCallback(new Callback<>(){
					...
					public void onSuccess(List<Status> result){
						...
						List<Filter> filters=AccountSessionManager.getInstance().getAccount(accountID).wordFilters.stream().filter(f->f.context.contains(Filter.FilterContext.PUBLIC)).collect(Collectors.toList());
						if(!filters.isEmpty()){
						...
						if(!toAdd.isEmpty()){
							...
							showNewPostsButton();
							AccountSessionManager.getInstance().getAccount(accountID).getCacheController().putPublicTimeline(toAdd, false);
						}
					}
	...
	public void onGapClick(GapStatusDisplayItem.Holder item){
		...
		dataLoading=true;
		dataLoading=true;
		currentRequest=new GetPublicTimeline(false, false, item.getItemID(), null, 20)
				.setCallback(new Callback<>(){
					@Override
					public void onSuccess(List<Status> result){
						currentRequest=null;
						...
						if(result.isEmpty()){
							...
							if(gapStatus!=null){
								...
								AccountSessionManager.getInstance().getAccount(accountID).getCacheController().putPublicTimeline(Collections.singletonList(gapStatus), false);
							}
						}else{
							...
							for(Status s:data){
								if(belowGap){
									...
									belowGap=true;
									s.hasGapAfter=false;
									AccountSessionManager.getInstance().getAccount(accountID).getCacheController().putPublicTimeline(Collections.singletonList(s), false);
								}else{
									gapPostIndex++;
								}
							}
							...
							AccountSessionManager.getInstance().getAccount(accountID).getCacheController().putPublicTimeline(insertedPosts, false);
						}
					}
